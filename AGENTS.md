...
- **A `.viz-box` MUST wrap its SVG host in an inner `div[id]`** — `<div class="viz-box"><div
  ...
  `div[id]`, which is how the `partition_*` tabs shipped with no fullscreen button on any of
  their panels. `dom_smoke` now asserts the shape for every
...
**A blank `atom_layout_mnk` means `(1, 1, 1)`, not an error.** Both it and `permutation_mnk` are
*optional* arguments of `cute.make_tiled_mma` — the signature is
`make_tiled_mma(op_or_atom, atom_layout_mnk=(1,1,1), permutation_mnk=None)` — so an empty box means
the argument was omitted and should resolve to the DSL's own default. `permutation_mnk`'s blank
already meant `None`; `atom_layout_mnk` used to throw, which made the two boxes disagree about what
empty means. Blank now gives `(1,1,1):(0,0,0)`, identical to typing it. The placeholder shows the
value so the box says what it will do. Shared with `partition_abc` through `mtmParseAtomLayout`,
so the MMA tabs agree.
